...
// 			<div class="relative">
// 				<div class="w-[100px] h-[100px] absolute top-[-18px] right-[415px] !m-2 flex flex-col items-center  !pr-4 !pt-3  shadow-lg backdrop-blur-md" style="clip-path: polygon(99% 0, 50% 51%, 0 100%,0 0%); background-color: ${colorTheme};">
...
// 					<span style="color: ${colorTheme}" id="score-left" class="text-3xl font-black  drop-shadow-lg">${this.score[0]}</span>
// 				</div>
...
// 					<span style="color: ${colorTheme}" id="score-right" class="text-3xl font-black text-amber-400 drop-shadow-lg">${this.score[1]}</span>
// 				</div>
// 			</div>


// 			<canvas id="canvas" class="border-2 border-white/20 rounded-xl !mb-10 h-[70%] w-full" style="background: ${colorTheme}; "></canvas>
...
export class LocalGameView extends View {
    ...
    // ✨ --- ADDED: colorTheme is now a class property ---
    ...
    // ... (rest of your methods like resetBall, checkRoundEnd, etc. remain unchanged) ...
    private resetBallSpeed() {
        this.dx = 2;
        this.dy = 2;
    }
    ...
    render(user: User | null): HTMLElement {
       this.user = user;
        const element = document.createElement('section');
        element.classList.add(
            'bg-[var(--secondary)]', 'rounded-4xl', 'border', 'border-white/10',
            'w-full', 'h-[80%]', '!mt-16', 'flex', 'flex-col', 'lg:flex-row',
            'items-center', 'lg:items-stretch', 'justify-between',
            'lg:!gap-0', '!p-2', 'overflow-hidden'
        );
        ...
        <div id="game-view" class="flex hidden flex-col items-center justify-between gap-2 w-[88%] h-full !m-auto">
            <div class="relative">
                <div class="w-[100px] h-[100px] absolute top-[-18px] right-[415px] !m-2 flex flex-col items-center  !pr-4 !pt-3  shadow-lg backdrop-blur-md" style="clip-path: polygon(99% 0, 50% 51%, 0 100%,0 0%); background-color: ${this.colorTheme};">
                    <span class="text-[10px] font-black text-[var(--secondary)] !ml-2 tracking-widest uppercase">Round</span>
                    <span id="round-display" class="text-xl font-bold text-[var(--secondary)] drop-shadow-md ">1</span>
                </div>
            </div>
            ...
            <div class="w-full h-[15%] flex items-center justify-between !px-6 rounded-2xl  border border-white/20 shadow-lg backdrop-blur-md !mt-5">
                ...
                <div class="flex flex-col items-center text-white w-[20%]">
                    <span id="user1" class="text-lg font-semibold tracking-wide">User 1</span>
                    <span style="color: ${this.colorTheme}" id="score-left" class="text-3xl font-black  drop-shadow-lg">${this.score[0]}</span>
                </div>
                ...
                <div class="flex flex-col items-center text-white w-[20%]">
                    <span id="user2" class="text-lg font-semibold tracking-wide">User 2</span>
                    <span style="color: ${this.colorTheme}" id="score-right" class="text-3xl font-black text-amber-400 drop-shadow-lg">${this.score[1]}</span>
                </div>
            </div>

            <canvas id="canvas" class="border-2 border-white/20 rounded-xl !mb-10 h-[70%] w-full " style="background: ${this.colorTheme}; "></canvas>
            <div id="goal-msg" class="absolute  hidden text-4xl font-bold" >goaal!</div>
            <div id="end-game" class="absolute hidden text-4xl border-white"></div>
            <div id="pause-game-icon" class="absolute hidden border-white bg-white/20 rounded-xl top-[54%] !m-auto">
                <i class="ti ti-player-pause-filled text-8xl text-white/80 drop-shadow-2xl"></i>
        ...
        <div id="forme-names" class="relative w-full h-full flex items-center justify-center">
            ...
            <div class="relative z-10 flex flex-col items-center gap-10 w-full h-full justify-center !mt-8">
                <div class="text-center">
                    <h1 class="text-6xl font-black text-transparent bg-clip-text drop-shadow-2xl !mb-4 bg-[var(--accent)]">
                        P<span class="text-white">ONG</span>P<span class="text-white">ONG</span>
                    </h1>
                    <p class="text-md text-white/60 font-medium">Enter player names to begin the ultimate showdown!</p>
                </div>
                ...
                <div class="flex flex-col lg:flex-row items-center gap-12 !">
                    <div class="relative group w-[350px] ">
                        ...
                        <div class="relative !p-6 flex flex-col justify-center items-center">
                            <div class="text-center !mb-4">
                                <div class="text-5xl !mb-2 flex justify-center items-center text-[var(--accent)]">
                                    <i class="ti ti-user "></i>
                                    <div class="rotate-180"><i class="ti ti-ping-pong text-2xl"></i></div>
                                </div>
                                <p class="text-[12px] text-white/70">Left Paddle (W/S)</p>
    ...
    private moveBall(){
        if(!this.canvas || !this.x || !this.y || this.xPaddleLeft === null || this.yPaddleLeft === null || this.xPaddleRight === null || this.yPaddleRight === null) return;
    ...
    private DisplayButtonPlay(){
        const playElement = document.getElementById("playbutton");
        if (!playElement || !this.canvas ) return;
        if (this.startFlag) {
            playElement.classList.add("hidden");
            this.canvas.style.backgroundColor = this.computedColor;
            this.canvas.classList.remove("animate-pulse");
        } else {
            if (!this.winnerFlag) {
                playElement.classList.remove("hidden");
    ...
    private moveBallHori(){
        if(!this.canvas || !this.x || this.xPaddleLeft === null || this.xPaddleRight === null) return;
        if(this.x + this.rayon >= this.xPaddleRight)
            this.dx *= -1;
        if(this.x - this.rayon <= this.xPaddleLeft + 10)
            this.dx *= -1;
    ...
    draw(): void {
        this.DisplayButtonPlay();
        if (!this.ctx || !this.canvas || !this.x || !this.y || this.xPaddleLeft === null || this.yPaddleLeft === null || this.xPaddleRight === null || this.yPaddleRight === null) return;
    ...
    onMount(): void {
        ...
        document.addEventListener('keydown', (event : KeyboardEvent) => {
            if(this.yPaddleLeft === null || this.yPaddleRight === null) return;
            switch(event.key) {
                case 'w':
                    if(this.yPaddleLeft > 0) this.yPaddleLeft -= 20;
                    break;
                case 's':
                    if(this.yPaddleLeft < 340) this.yPaddleLeft += 20;
                    break;
                case 'ArrowUp':
                    if(this.yPaddleRight > 0) this.yPaddleRight -= 20;
                    break;
                case 'ArrowDown':
                    if(this.yPaddleRight < 340) this.yPaddleRight += 20;
                    break;
            }
        });
        const playButton = document.getElementById("playbutton");
